...
/**
 * Implementation of the AirportProvider based on ourAirports. To use this provider make sure you
...
public final class OurAirportsAirportProvider implements AirportProvider {
  ...
  public OurAirportsAirportProvider()
      throws CsvValidationException, IOException, URISyntaxException {
  ...
  public void buildCountries() throws URISyntaxException, IOException, CsvValidationException {
    ...
    try (InputStream countriesStream = countriesUri.toURL().openStream();
        CSVReader reader =
            new CSVReaderBuilder(new InputStreamReader(countriesStream, StandardCharsets.UTF_8))
  ...
  public void buildAirport() throws URISyntaxException, IOException, CsvValidationException {
    ...
    try (InputStream airportStream = airportsURI.toURL().openStream();
        CSVReader reader =
            new CSVReaderBuilder(new InputStreamReader(airportStream, StandardCharsets.UTF_8))
                .withCSVParser(parser)
                .withSkipLines(1)
                .build()) {
      String[] line;
