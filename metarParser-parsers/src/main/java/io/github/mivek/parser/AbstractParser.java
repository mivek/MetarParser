 ...
 * @author mivek Abstract class for Parser.
 */
public abstract class AbstractParser<T extends AbstractWeatherCode> {
  ...
  private static final Pattern TOKENIZE_REGEX =
      Pattern.compile("\\s((?=\\d/\\dSM)(?<!\\s\\d\\s)|(?!\\d/\\dSM))|=\\z");
   ...
   * @return a weather condition with its intensity, its descriptor and the phenomenon.
   */
  ...
  /**
   * Splits a string between spaces except if the space is between two digits with SM.
   *
