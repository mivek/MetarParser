...
public final class TAFParser extends AbstractParser<TAF> {
  ...
  private String[][] extractLineTokens(final String tafCode) {
    ...
    String[] lines =
        cleanedInput
            .replaceAll("\\s(PROB\\d{2}\\sTEMPO|TEMPO|INTER|BECMG|FM|PROB)", "\n$1")
            .split("\n");
    ...
    if (lineTokens.length > 1) {
      ...
      List<String> temperatures =
          Arrays.stream(last)
              .filter(code -> code.startsWith(TX) || code.startsWith(TN))
              .collect(Collectors.toList());
      if (!temperatures.isEmpty()) {
        lineTokens[0] =
            Stream.concat(Arrays.stream(lineTokens[0]), temperatures.stream())
                .toArray(String[]::new);
        lineTokens[lines.length - 1] =
            Arrays.stream(last)
                .filter(code -> !code.startsWith(TX) && !code.startsWith(TN))
                .toArray(String[]::new);
      }
  ...
  /**
   * Parse the validity part of a {@link TAFParser} or an {@link AbstractTafTrend}.
   *
   ...
   */
  private void iterChanges(
      final int index, final String[] parts, final AbstractTafTrend<Validity> change) {
