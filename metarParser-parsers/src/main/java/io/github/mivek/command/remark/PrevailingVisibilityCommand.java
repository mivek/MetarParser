...
public final class PrevailingVisibilityCommand implements Command {
  ...
  private static final Pattern VARIABLE_PREVAILING_VISIBILITY =
      Pattern.compile("^VIS ((\\d)*( )?(\\d?/?\\d))V((\\d)*( )?(\\d?/?\\d))");
  ...
  public String execute(final String remark, final StringBuilder stringBuilder) {
    ...
    stringBuilder
        .append(
            messages.getString(
                "Remark.Variable.Prevailing.Visibility", visibilityParts[1], visibilityParts[5]))
        .append(" ");
