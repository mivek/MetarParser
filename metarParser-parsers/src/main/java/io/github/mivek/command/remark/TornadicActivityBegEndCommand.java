...
public final class TornadicActivityBegEndCommand implements Command {
  ...
  private static final Pattern TORNADIC_ACTIVITY_BEG_END =
      Pattern.compile(
          "^(TORNADO|FUNNEL CLOUD|WATERSPOUT) (B(\\d{2})?(\\d{2}))(E(\\d{2})?(\\d{2}))( (\\d+)?"
              + " ([A-Z]{1,2})?)?");
    ...
    stringBuilder
        .append(
            messages.getString(
                ...
                messages.getString("Remark." + tornadicParts[1].replace(" ", "")),
                verifyString(tornadicParts[3]),
                tornadicParts[4],
                verifyString(tornadicParts[6]),
                tornadicParts[7],
                tornadicParts[9],
                messages.getString("Converter." + tornadicParts[10])))
        .append(" ");
