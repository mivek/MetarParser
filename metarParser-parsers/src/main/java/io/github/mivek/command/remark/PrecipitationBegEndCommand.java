...
public final class PrecipitationBegEndCommand implements Command {
  ...
  private static final Pattern PRECIPITATION_BEG_END =
      Pattern.compile("^(([A-Z]{2})?([A-Z]{2})B(\\d{2})?(\\d{2})E(\\d{2})?(\\d{2}))");
    ...
    stringBuilder
        .append(
            messages.getString(
                ...
                verifyString(precipitationBegEnd[6]),
                precipitationBegEnd[7]))
        .append(" ");
