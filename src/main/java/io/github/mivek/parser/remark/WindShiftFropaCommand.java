 ...
 * @author mivek
 */
public class WindShiftFropaCommand implements Command {
    ...
    @Override public final String execute(final String pRemark, final StringBuilder pStringBuilder) {
        String[] windShiftParts = Regex.pregMatch(RemarkParser.WIND_SHIFT_FROPA, pRemark);
        pStringBuilder.append(fMessages.getString("Remark.WindShift.FROPA", verifyString(windShiftParts[1]), windShiftParts[2]));
        pStringBuilder.append(" ");
        return pRemark.replaceFirst(RemarkParser.WIND_SHIFT_FROPA.pattern(), "");
    }
}
